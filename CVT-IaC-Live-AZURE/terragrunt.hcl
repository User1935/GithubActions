...
data "sops_file" "secrets" {
  source_file = join("/",["",split("/",abspath(path.root))[1],split("/",abspath(path.root))[2],"secure/stuff.yaml"]) #join("/",["",split("/",abspath(path.root))[1],split("/",abspath(path.root))[2],"secure/stuff.yaml"]) #join("/",["",local.sops_file_path[1],local.sops_file_path[2],local.sops_file_path[3],local.sops_file_path[4], local.sops_file_path[5],"secure/stuff.yaml"]) 
  input_type  = "yaml"
}
EOF
}
